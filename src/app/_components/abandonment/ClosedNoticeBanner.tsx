...
/**
 * 보호가 끝났거나 공고 기간이 지난 유기동물 상세에 띄우는 안내.
 *
 ...
 *  - 반대로 기간만 지난 아이에게는 상태를 아는 척하게 된다.
 *
 * 그래서 `processState` 가 `종료(사유)` 면 그 사유를 그대로 전하고, 사유를 모르면(공고기간 만료,
 * 상류에서 사라짐) **추측하지 않고 보호소 확인으로 넘긴다.** 공고 후에도 보호소가 계속 데리고
...
export default function ClosedNoticeBanner({
  ...
  const title =
    known?.title ?? (reason ? `보호가 종료되었습니다 (${reason})` : "공고 기간이 종료되었습니다");
