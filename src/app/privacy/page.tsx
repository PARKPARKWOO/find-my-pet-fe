...
/** 최초 제정일. 개정으로 시행일이 밀려도 "언제부터 있던 방침인가" 는 고정이라 따로 둔다. */
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-8 pb-24">
      ...
      <Section id="retention" no={4} title="개인정보의 보유·이용 기간 및 파기">
        ...
        <Bullets
          items={[
            <span key="b2">
              ...
              배치 처리를 운영하고 있지 않습니다.</b> 회원 정보와 게시물은 이용자가 직접 삭제하거나
              회원 탈퇴할 때, 또는 삭제 요청이 있을 때 처리됩니다.
            </span>,
        ...
        <LegalSubHeading>회원 탈퇴 시의 처리</LegalSubHeading>
        <p>
          이용자는 <b className="text-foreground">마이페이지에서 직접 회원 탈퇴</b>를 할 수 있으며,
          탈퇴를 확정하면 다음 처리가 즉시 실행됩니다. 실종 신고 게시글에는 보호자의 휴대전화번호가
          공개된 상태로 들어 있으므로, 탈퇴 시 게시물을 남겨 두지 않고 함께 파기합니다. 탈퇴 화면에서
          삭제될 항목의 건수를 먼저 확인할 수 있습니다.
        </p>
        <Table
          head={["대상", "처리 방식"]}
          rows={[
            [
              "실종 신고 게시글",
              "휴대전화번호, 실종 장소, 좌표, 상세 설명, 카카오 오픈채팅 주소를 빈 값으로 덮어쓰고 제목과 작성자명을 '삭제된 게시글'·'탈퇴한 사용자'로 바꾼 뒤 삭제 표시를 기록합니다. 서비스 화면과 API 응답에서 제외됩니다.",
            ],
            [
              "게시글 사진",
              "삭제 표시를 기록합니다. 저장소의 원본 파일은 남을 수 있으나 조회 주소는 1시간 뒤 만료됩니다.",
            ],
            [
              "목격 제보",
              "제보 위치와 시각은 아직 반려동물을 찾고 있는 다른 이용자의 단서이므로 남기고, 제보자 닉네임·메모·사진과 회원 연결을 끊어 익명 처리합니다.",
            ],
            ["전단지 부착 위치", "메모를 삭제하고 삭제 표시를 기록합니다."],
            [
              "즐겨찾기·관심 지역 구독",
              "삭제 표시를 기록하며, 이후 관련 알림이 발송되지 않습니다.",
            ],
            ["받은 알림", "제목·내용·연결 링크를 삭제한 뒤 삭제 표시를 기록합니다."],
            ["작성한 후기", "데이터베이스에서 문서 자체를 삭제합니다."],
            [
              "회원 정보(카카오 회원번호·닉네임·이메일)",
              "회원 인증 서버에 삭제 표시를 기록하여 로그인과 조회 대상에서 제외합니다. 저장된 값 자체를 지우는 물리적 삭제는 아닙니다.",
            ],
            [
              "카카오 로그인 토큰",
              "저장된 토큰을 삭제하고 카카오에 연결 해제를 요청합니다. 토큰이 이미 만료되어 연결 해제 요청이 실패하더라도 탈퇴는 그대로 완료되며, 이 경우 카카오 계정 설정에서 직접 연결을 해제하실 수 있습니다.",
            ],
          ]}
          minWidth={560}
        />
        <Bullets
          items={[
            "회원 인증 서버는 운영자가 함께 운영하는 다른 서비스와 공용입니다. 같은 카카오 계정으로 다른 서비스를 이용 중인 경우 파인드마이펫 연결만 해제되고 회원 정보와 카카오 연결은 유지되며, 남은 연결이 없을 때 위 표와 같이 처리됩니다.",
            "탈퇴 후 같은 카카오 계정으로 다시 로그인하면 이전 계정과 연결되지 않는 새 회원으로 가입되며, 탈퇴 전의 데이터는 복구되지 않습니다.",
            "위 처리는 삭제 표시와 원문 삭제를 함께 수행하는 것이며, 삭제 표시가 기록된 행 자체는 데이터베이스에 남습니다. 이 잔여 기록의 완전 삭제를 원하시면 제12조의 문의처로 요청해 주십시오.",
          ]}
        />
      </Section>
      ...
      <Section id="rights" no={8} title="정보주체와 법정대리인의 권리와 행사 방법">
        ...
        <Bullets
          items={[
            ...
            <span key="b6">
              <b className="text-foreground">회원 탈퇴.</b> 마이페이지 하단의 &lsquo;회원 탈퇴&rsquo;에서
              직접 하실 수 있습니다. 확인 화면에서 삭제될 항목의 건수를 미리 확인할 수 있으며, 확정하면
              제4조 &lsquo;회원 탈퇴 시의 처리&rsquo;가 즉시 실행됩니다. 되돌릴 수 없습니다.
            </span>,
          ]}
        />
        <LegalSubHeading>
        ...
        <Bullets
          items={[
            ...
            "삭제 표시만 남아 있는 게시물·제보 기록과 탈퇴 후 남은 회원 정보의 완전 삭제",
            "개인정보 처리의 정지 요구",
          ]}
        />
        <p>
      ...
      <Section id="officer" no={12} title="개인정보 보호책임자 및 문의처">
        ...
        <Table
          ...
          rows={[
            ...
            [
              "접수 범위",
              "개인정보 열람·정정·삭제·처리정지 요구, 게시물 삭제 요청, 탈퇴 후 남은 기록의 완전 삭제 요청, 개인정보 관련 문의 및 신고 (회원 탈퇴 자체는 마이페이지에서 직접 하실 수 있습니다)",
            ],
          ]}
          minWidth={420}
        />
      ...
      <Section id="changes" no={14} title="처리방침의 변경">
        ...
        <div className="rounded-lg border p-4">
          ...
          <ul className="mt-2 space-y-1">
            <li>
              {EFFECTIVE_DATE} — 마이페이지 회원 탈퇴 기능 도입에 따라 제4조(회원 탈퇴 시의 처리
              신설), 제8조(탈퇴를 서비스 내 직접 처리 항목으로 이동), 제12조(접수 범위) 개정
            </li>
